...
#define RMI_54_STOP_ANTITOUCH_TIME	(30*HZ)
...
show_store_union_struct_prototype(self_test)
...
#define TEST_RESULT_MAX_LENGTH	10
struct rmi_fn_54_data {
	...
	unsigned char test_result[TEST_RESULT_MAX_LENGTH];
};
...
static bool rmi_fn_54_test_data_check(struct rmi_fn_54_data *rmi_data, int index)
{
	bool result = true;
	short val = 0;
	struct rmi_function_container *fc = rmi_data->fc;
	unsigned int i = 0;
	short max, min;
	short* standard_data;
	int len = 0;

	standard_data = (short*)rmi_data->test_data[index].data;
	len = (int)(rmi_data->test_data[index].size / sizeof(short));

	for (i = 0; i < len; i+=2) {
		val = (rmi_data->report_data[i+1] << 8) | rmi_data->report_data[i];
...
static ssize_t rmi_fn_54_get_report_store(struct device *dev,
	...
	mutex_lock(&instance_data->status_mutex);
	if (instance_data->status == BUSY) {
		dev_err(dev, "Device is in busy state!\n");
		mutex_unlock(&instance_data->status_mutex);
		return -EBUSY;
	}
	/* Store interrupts */
	...
	/* prelong the time to 5 seconds */
	hrtimer_start(&instance_data->watchdog, ktime_set(5, 0),
							HRTIMER_MODE_REL);
#endif
	/* add code here to wait for the data ready (interrupt) */
	while (1) {
		msleep(20);
		if (instance_data->status != BUSY)
			break;
	...
	return ((instance_data->status == IDLE) ?
			count : instance_data->status);
}

void rmi_get_proper_test_data(struct rmi_fn_54_data* data)
{
	int i;
	struct rmi_device_platform_data *pdata;
	pdata = to_rmi_platform_data(data->fc->rmi_dev);
	dev_info(&data->fc->dev, "size = %d\n", pdata->config_array_size);
	for (i = 0; i < pdata->config_array_size; i++) {
		dev_info(&data->fc->dev, "%s %s\n", pdata->config_info_array[i].product_name, data->fc->rmi_dev->product_id);
		if (strcmp(pdata->config_info_array[i].product_name,
			data->fc->rmi_dev->product_id) == 0) {
			data->test_data = pdata->config_info_array[i].self_test_data;
			dev_info(&data->fc->dev, "id choose = %d\n", i);
			return;
		}
	}
	data->test_data = NULL;
...
static ssize_t rmi_fn_54_self_test_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	fc = to_rmi_function_container(dev);
	f54 = fc->data;
	rmi_get_proper_test_data(f54);
	/* do test */
	memcpy(f54->test_result, "0", strlen("0"));
	for (i = 0; i < sizeof(test_array) / sizeof(char*); i++) {
		dev_info(&fc->dev, "test start at %d\n", i);
		error = rmi_fn_54_report_type_store(dev, NULL,
			test_array[i], strlen(test_array[i]));
		if (error <= 0)
			return error;
		error = rmi_fn_54_get_report_store(dev, NULL,
			buf, count);
		if (error <= 0)
			return error;
		result = rmi_fn_54_test_data_check(f54, i);
		if (!result) {
...
static ssize_t rmi_fn_54_self_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct rmi_function_container *fc;
	struct rmi_fn_54_data *f54;
