...
static int rmi_f1a_initialize(struct rmi_function_container *fc)
{
	...
	if (f1a->sensor_button_count != pdata->f1a_button_map->nbuttons) {
		dev_warn(&fc->dev, "firmware button count value = %d is"
		"not equal to the real button number = %d",
		f1a->sensor_button_count,
		pdata->f1a_button_map->nbuttons);
		f1a->sensor_button_count = pdata->f1a_button_map->nbuttons;
	}
