...
{
	int retval;
	bool f01_found = false;
	bool f34_found = false;
...
static int rmi_f34_rescan_pdt(struct reflash_data *data)
{
	int i;
	int retval;
	struct rmi_device *rmi_dev = data->rmi_dev;
	...
	for (i = 0; i < RETRY_COUNT; i++) {
		dev_info(&rmi_dev->dev, "rescan pdt for the %d time\n", i);
		retval = rescan_pdt(data);
		if (!retval) {
			dev_info(&rmi_dev->dev, "Rescan PDT successfully!\n");
			break;
		}
		msleep(RETRY_SLEEP);
	}
...
static int enter_flash_programming(struct reflash_data *data)
{
	...
	retval = rmi_f34_rescan_pdt(data);
	if (retval)
		return retval;
...
static bool go_nogo(struct reflash_data *data, struct image_header *header)
{
	...
	/* add id array */
	u8 id[4];
	u8 control_base_addr = data->f34_pdt->control_base_addr;
	struct rmi_device *rmi_dev = data->rmi_dev;

	dev_info(&rmi_dev->dev, "addr = 0x%x\n", control_base_addr);
	dev_info(&rmi_dev->dev, "go_nogo\n");

	/* compare current id with firmware id, if not equal,
	flash it */
	retval = rmi_read_block(rmi_dev, control_base_addr, id,
		ARRAY_SIZE(id));
	if (retval < 0)
		dev_err(&rmi_dev->dev,
			"Couldn't read config id from 0x%x, error = %d",
			control_base_addr, retval);
	else {
		dev_info(&rmi_dev->dev,
			"original id = %#04x, %#04x, %#04x, %#04x",
			id[0], id[1], id[2], id[3]);
		dev_info(&rmi_dev->dev,
			"current  id =  %#04x, %#04x, %#04x, %#04x",
			data->config_data[0], data->config_data[1],
			data->config_data[2], data->config_data[3]);
		if (memcmp(id, data->config_data, ARRAY_SIZE(id))) {
			dev_info(&rmi_dev->dev,
