...
static irqreturn_t rmi_i2c_irq_thread(int irq, void *p)
{
	...
	if (lkl_rmi_get_attn_status() == pdata->attn_polarity) {
				  ...
				  const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "Probing %s at %#02x (IRQ %d).\n",
		...
		client->addr, pdata->attn_gpio_irq);
	...
	/* before use i2c we should bind gpio with power pin*/
