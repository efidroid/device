...
#include <linux/i2c.h>
#include <linux/device/atmel_mxt_ts.h>
...
int mxt_platform_get_lcd_id(void);

static int mxt_get_lcd_id(struct mxt_data * data)
{
#if 1
    return mxt_platform_get_lcd_id();
#else
	int error;
	int ret;
...
{
	struct mxt_data *data = dev_get_drvdata(dev);
	return kstrtoul(buf, 0, &data->dbgdump) < 0 ? 0 : count;
}
...
int mxt_platform_reset_set(int on);
...
static void mxt_reset_toggle(struct mxt_data *data)
{
	...
	for (i = 0; i < 10; i++) {
        mxt_platform_reset_set(1);
		//gpio_set_value_cansleep(pdata->reset_gpio, 1);
		msleep(1);
        mxt_platform_reset_set(0);
...
int mxt_platform_read_irq_gpio(void);
...
static int mxt_is_irq_assert(struct mxt_data *data)
{
	...
	if (pdata->irqflags & IRQF_TRIGGER_LOW)
		return mxt_platform_read_irq_gpio() == 0;
	else
		return mxt_platform_read_irq_gpio() == 1;
}
